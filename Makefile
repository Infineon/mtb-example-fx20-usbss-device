...
APPNAME=mtb-example-fx20-usbss-device
...
# Use FreeRTOS unless RTOS_EN=no switch is used.
# NOTE: If unused, also remove `FREERTOS` and `RTOS_AWARE` from the
# `COMPONENTS` make variable.
RTOS_EN?=yes
ifeq ($(RTOS_EN), yes)
    DEFINES += FREERTOS_ENABLE=1
else
    DEFINES += FREERTOS_ENABLE=0
endif

# When using the SRCSINK=yes switch, IN endpoints continuously provide data.
# This data is continuously drained by OUT endpoints.
SRCSINK?=no
ifeq ($(SRCSINK), yes)
    DEFINES += APP_SRC_SNK_EN=1
else
    DEFINES += APP_SRC_SNK_EN=0
endif

# Compile to bind with WinUSB driver unless CYUSB=yes switch is used.
CYUSB?=no
ifeq ($(CYUSB), yes)
